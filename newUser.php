        ...
        while ($row = $res->fetch_assoc()) {
          ...
          break;
			...
			if ($exists == false) {
				...
				if ($stat = $connection->prepare("insert into user(username, fname, lname, email, pass, quest, ans, profilePic) values (?,?,?,?,?,?,?,?)") ) {
          ...
          // if everything is ok, try to upload file
            ...
            if (move_uploaded_file($_FILES["profilePic"]["tmp_name"], $target_file)) {
              $pass = md5($pass);
              $stat->bind_param("ssssssss", $user, $fname, $lname, $email, $pass, $question, $answer, $profile_pic);
              if ($stat->execute()) {
